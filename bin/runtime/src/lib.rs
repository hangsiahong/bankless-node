...
use primitives::{ApiError as BanklessApiError, AuthorityId as BanklessId};
...
pub mod opaque {
    ...
    impl_opaque_keys! {
        pub struct SessionKeys {
            pub aura: Aura,
            pub bankless: Bankless,
        }
    }
}

pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("bankless-node"),
    impl_name: create_runtime_str!("bankless-node"),
    authoring_version: 1,
    spec_version: 6,
    impl_version: 1,
...
impl MinimumPeriod {
    pub fn get() -> u64 {
        Bankless::millisecs_per_block() / 2
    }
}
...
impl pallet_bankless::Config for Runtime {
    type AuthorityId = BanklessId;
    type Event = Event;
}
...
impl pallet_session::Config for Runtime {
    ...
    type SessionManager = pallet_bankless::BanklessSessionManager<Self>;
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic
    {
        ...
        Bankless: pallet_bankless::{Pallet, Call, Config<T>, Storage, Event<T>},
...
impl_runtime_apis! {
    ...
    impl primitives::BanklessSessionApi<Block> for Runtime {
        fn next_session_authorities() -> Result<Vec<BanklessId>, BanklessApiError> {
            Bankless::next_session_authorities()
        }

        fn authorities() -> Vec<BanklessId> {
            Bankless::authorities()
        }
