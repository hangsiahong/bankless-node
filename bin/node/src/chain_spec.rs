use bankless_primitives::{
    AuthorityId as BanklessId, ADDRESSES_ENCODING, DEFAULT_MILLISECS_PER_BLOCK,
    DEFAULT_SESSION_PERIOD, TOKEN_DECIMALS,
};
use bankless_runtime::{
    AccountId, BanklessConfig, AuraConfig, BalancesConfig, GenesisConfig, SessionConfig, SessionKeys,
    Signature, SudoConfig, SystemConfig, VestingConfig, WASM_BINARY,
};
use finality_bankless::{MillisecsPerBlock, SessionPeriod};
use libp2p::PeerId;
...
pub struct ChainParams {
    ...
    /// Chain name. Default is "Bankless World Development"
    #[structopt(long, default_value = "Bankless World Development")]
    chain_name: String,
