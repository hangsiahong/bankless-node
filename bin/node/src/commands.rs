...
use bankless_primitives::AuthorityId as BanklessId;
use bankless_runtime::AccountId;
...
/// returns Bankless key, if absent a new key is generated
fn bankless_key(keystore: &impl SyncCryptoStore) -> BanklessId {
    SyncCryptoStore::ed25519_public_keys(&*keystore, bankless_primitives::KEY_TYPE)
        .pop()
        .unwrap_or_else(|| {
            SyncCryptoStore::ed25519_generate_new(&*keystore, bankless_primitives::KEY_TYPE, None)
                .expect("Could not create Bankless key")
        })
        .into()
}
...
fn authority_keys(
    ...
    let bankless_key = bankless_key(keystore);
