...
function usage(){
  echo "Usage:
      ./run_nodes.sh [-v N_VALIDATORS] [-n N_NON_VALIDATORS] [-b false] [-p BASE_PATH] [BANKLESS_NODE_ARG]...
...
killall -9 bankless-node

set -e

clear

if $BUILD_BANKLESS_NODE ; then
  cargo build --release -p bankless-node
fi
...
./target/release/bankless-node bootstrap-chain --millisecs-per-block 2000 --session-period 40 --base-path "$BASE_PATH" --account-ids "$validator_ids_string" --chain-type local > "$BASE_PATH/chainspec.json"

for i in $(seq "$N_VALIDATORS" "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
  echo "Bootstrapping node $i"
  account_id=${account_ids[$i]}
  ./target/release/bankless-node bootstrap-node --base-path "$BASE_PATH" --account-id "$account_id" --chain-type local
done

bootnodes=""
for i in 0 1; do
    pk=$(./target/release/bankless-node key inspect-node-key --file $BASE_PATH/${account_ids[$i]}/p2p_secret)
...
for i in $(seq 0 "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
  auth=node-$i
  account_id=${account_ids[$i]}
  ./target/release/bankless-node purge-chain --base-path $BASE_PATH/$account_id --chain $BASE_PATH/chainspec.json -y
  ./target/release/bankless-node \
    --validator \
