...
/// The data ordered by the Bankless consensus.
#[derive(Copy, PartialEq, Eq, Clone, Debug, Encode, Decode, Hash)]
pub struct BanklessData<H, N> {
    pub hash: H,
    pub number: N,
}

impl<H, N> BanklessData<H, N> {
...
pub(crate) type BanklessDataFor<B> = BanklessData<<B as BlockT>::Hash, NumberFor<B>>;

/// A trait allowing to check the data contained in an BanklessBFT network message, for the purpose of
/// data availability checks.
pub trait BanklessNetworkMessage<B: BlockT> {
...
/// This component is used for filtering available data for Bankless Network.
...
where
    ...
    Message: BanklessNetworkMessage<B> + std::fmt::Debug,
{
    next_message_id: MessageId,
    ready_messages_tx: UnboundedSender<Message>,
    messages_rx: UnboundedReceiver<Message>,
    missing_blocks: HashMap<BanklessDataFor<B>, MissingBlockInfo>,
    available_blocks: LruCache<BanklessDataFor<B>, ()>,
    ...
    Message: BanklessNetworkMessage<B> + std::fmt::Debug,
{
    ...
    /// 1. Receives BanklessNetworkMessage and either sends it further if message is available or saves it for later
        ...
        loop {
            tokio::select! {
                ...
                Some(block) = &mut import_stream.next() => {
                    ...
                    self.add_block(BanklessData::new(block.hash, *block.header.number()));
                }
    ...
    fn add_pending_message(&mut self, message: Message, requirements: Vec<BanklessDataFor<B>>) {
    ...
    fn push_messages(&mut self, block_data: BanklessDataFor<B>) {
    ...
    fn add_block(&mut self, block_data: BanklessDataFor<B>) {
...
/// Provides data to BanklessBFT for ordering.
#[derive(Clone)]
pub struct DataProvider<B: BlockT> {
    pub(crate) proposed_block: Arc<Mutex<BanklessDataFor<B>>>,
        ...
        tokio::select! {
            _ = delay => {
                ...
                if *new_best_header.number() <= max_block_num {
                    *proposed_block.lock() = BanklessData::new(new_best_header.hash(), *new_best_header.number());
                } else {
                    ...
                    if proposed_block.lock().number < max_block_num {
                        ...
                        *proposed_block.lock() = BanklessData::new(reduced_header.hash(), *reduced_header.number());
                    } else {
...
#[async_trait]
impl<B: BlockT> bankless_bft::DataProvider<BanklessDataFor<B>> for DataProvider<B> {
    async fn get_data(&mut self) -> BanklessDataFor<B> {
...
/// Accepts and forwards units ordered by BanklessBFT.
pub struct FinalizationHandler<B: BlockT> {
    pub(crate) ordered_units_tx: mpsc::UnboundedSender<BanklessDataFor<B>>,
}

#[async_trait]
impl<B: BlockT> bankless_bft::FinalizationHandler<BanklessDataFor<B>> for FinalizationHandler<B> {
    async fn data_finalized(&mut self, data: BanklessDataFor<B>) {
