use crate::data_io::{BanklessData, BanklessDataFor, BanklessNetworkMessage, DataStore, DataStoreConfig};
...
struct TestBlockRequester<B: BlockT> {
    blocks: mpsc::UnboundedSender<BanklessDataFor<B>>,
    justifications: mpsc::UnboundedSender<BanklessDataFor<B>>,
}

impl<B: BlockT> TestBlockRequester<B> {
    fn new() -> (
        Self,
        mpsc::UnboundedReceiver<BanklessDataFor<B>>,
        mpsc::UnboundedReceiver<BanklessDataFor<B>>,
    ) {
...
impl<B: BlockT> RequestBlocks<B> for TestBlockRequester<B> {
    ...
    fn request_stale_block(&self, hash: B::Hash, number: NumberFor<B>) {
        self.blocks
            .unbounded_send(BanklessData { hash, number })
            .unwrap();
    }
}
...
struct TestNetworkData {
    data: Vec<BanklessDataFor<Block>>,
}

impl BanklessNetworkMessage<Block> for TestNetworkData {
    fn included_blocks(&self) -> Vec<BanklessDataFor<Block>> {
        self.data.clone()
    }
}
...
struct DataStoreChannels {
    ...
    block_requests_rx: UnboundedReceiver<BanklessDataFor<Block>>,
...
fn prepare_data_store() -> (impl Future<Output = ()>, Arc<TestClient>, DataStoreChannels) {
    ...
    (
        ...
        client,
        DataStoreChannels {
            store_tx: bankless_network_tx,
            store_rx: bankless_network_rx,
            block_requests_rx,
            exit_data_store_tx,
        },
    )
...
async fn import_blocks(
    ...
    for _ in 0..n {
        ...
        blocks.push(BanklessData::new(block.header.hash(), block.header.number));
    }
    blocks
}
...
async fn sends_messages_after_import() {
    ...
    let data = BanklessData::new(block.header.hash(), block.header.number);
...
async fn does_not_send_messages_without_import() {
    ...
    let data = BanklessData::new(imported_block.header.hash(), imported_block.header.number);
    store_tx
        .unbounded_send(TestNetworkData { data: vec![data] })
        .unwrap();
