use crate::{
    ...
    justification::{BanklessJustification, JustificationNotification},
    metrics::Checkpoint,
    network::RmcNetwork,
...
pub struct IO<B: Block> {
    pub ordered_units_from_bankless: mpsc::UnboundedReceiver<BanklessDataFor<B>>,
...
{
    let IO {
        mut ordered_units_from_bankless,
    ...
    loop {
        tokio::select! {
            maybe_unit = ordered_units_from_bankless.next() => {
                if let Some(new_block_data) = maybe_unit {
                    trace!(target: "bankless-party", "Received unit {:?} in aggregator.", new_block_data);
                    if last_block_seen {
                    ...
                    debug!(target: "bankless-party", "Units ended in aggregator. Terminating.");
                    break;
                }
            }
            multisigned_hash = aggregator.next_multisigned_hash() => {
                if let Some((hash, multisignature)) = multisigned_hash {
                    ...
                    let notification = JustificationNotification {
                        justification: BanklessJustification{signature: multisignature},
                        hash,
                        number
                    };
                    if let Err(e) = justifications_for_chain.unbounded_send(notification)  {
                        error!(target: "bankless-party", "Issue with sending justification from Aggregator to JustificationHandler {:?}.", e);
                    }
                } else {
                    debug!(target: "bankless-party", "The stream of multisigned hashes has ended. Terminating.");
                    break;
                }
            }
            _ = &mut exit_rx => {
                debug!(target: "bankless-party", "Aggregator received exit signal. Terminating.");
                return;
            }
        }
    }
    debug!(target: "bankless-party", "Aggregator awaiting an exit signal.");
    ...
    BE: Backend<B> + 'static,
{
    ...
    let task = {
        async move {
            ...
            debug!(target: "bankless-party", "Running the aggregator task for {:?}", session_id);
            run_aggregator(aggregator, io, client, last_block, metrics, exit).await;
            debug!(target: "bankless-party", "Aggregator task stopped for {:?}", session_id);
        }
    };

    let handle = spawn_handle.spawn_essential("bankless/consensus_session_aggregator", task);
