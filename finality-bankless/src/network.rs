use bankless_bft::{Index, KeyBox as _, NodeIndex, SignatureSet};
...
use crate::{
    ...
    data_io::BanklessDataFor,
    Error, Hasher, SessionId,
};
use sp_api::NumberFor;
use std::{fmt::Debug, future::Future};

pub use crate::new_network::BanklessNetworkData;
...
/// own network.
pub(crate) const BANKLESS_PROTOCOL_NAME: &str = "/cardinals/bankless/1";
...
impl From<bankless_bft::Recipient> for Recipient<NodeIndex> {
    fn from(recipient: bankless_bft::Recipient) -> Self {
        match recipient {
            bankless_bft::Recipient::Everyone => Recipient::All,
            bankless_bft::Recipient::Node(node) => Recipient::Target(node),
        }
    }
}
        ...
        match sc {
            ...
            Control(control_command) => match control_command {
                ControlCommand::Terminate(session_id) => {
                    debug!(target: "afa", "Cleaning up after session {:?} in bankless network", session_id);
        ...
        loop {
            tokio::select! {
                ...
                _ = status_ticker.next() => {
                    debug!(target: "afa", "Total peers in bankless network {:?}", self.peers.all_peers.len());
...
pub(crate) enum NetworkData<B: BlockT> {
    Bankless(BanklessNetworkData<B>),
...
pub struct BanklessNetwork<B: BlockT> {
    inner: DataNetwork<BanklessNetworkData<B>>,
}

/// A network for sending messages related to the BanklessBFT protocol.
impl<B: BlockT> BanklessNetwork<B> {
    pub(crate) fn new(inner: DataNetwork<BanklessNetworkData<B>>) -> Self {
        BanklessNetwork { inner }
    }
}

#[async_trait::async_trait]
impl<B: BlockT> bankless_bft::Network<Hasher, BanklessDataFor<B>, Signature, SignatureSet<Signature>>
    for BanklessNetwork<B>
{
    fn send(&self, data: BanklessNetworkData<B>, recipient: bankless_bft::Recipient) {
    ...
    async fn next_event(&mut self) -> Option<BanklessNetworkData<B>> {
...
pub(crate) fn split_network<B: BlockT>(
    ...
    data_store_tx: mpsc::UnboundedSender<BanklessNetworkData<B>>,
    data_store_rx: mpsc::UnboundedReceiver<BanklessNetworkData<B>>,
) -> (BanklessNetwork<B>, RmcNetwork<B>, impl Future<Output = ()>) {
    let (rmc_data_tx, rmc_data_rx) = mpsc::unbounded();
    let (bankless_cmd_tx, bankless_cmd_rx) = mpsc::unbounded();
    let (rmc_cmd_tx, rmc_cmd_rx) = mpsc::unbounded();
    let bankless_network = BanklessNetwork::new(DataNetwork::new(
    ...
    let forward_data = async move {
        loop {
            match data_from_consensus_network.next().await {
                None => break,
                Some(NetworkData::Bankless(data)) => {
    ...
    let forward_bankless_cmd = {
        let cmd_tx = cmd_tx.clone();
        bankless_cmd_rx
            .map(|cmd| Ok(cmd.map(NetworkData::Bankless)))
            .forward(cmd_tx)
            .map(|res| {
                if let Err(e) = res {
                    warn!(target: "afa", "error forwarding bankless commands: {}", e);
                }
            })
    };
    let forward_rmc_cmd = {
    ...
    let forwards = futures::future::join3(forward_data, forward_bankless_cmd, forward_rmc_cmd)
        .map(|((), (), ())| ());
    (bankless_network, rmc_network, forwards)
}
