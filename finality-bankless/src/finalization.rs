...
use crate::data_io::BanklessDataFor;
...
pub(crate) struct BanklessFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
...
impl<B, BE, C> BanklessFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
    C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
...
impl<B, BE, C> BlockFinalizer<B> for BanklessFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
    C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
...
/// Given hash `last_finalized` and `BanklessDataFor` `new_data` of two blocks, returns
...
pub(crate) fn should_finalize<B, C>(
    ...
    new_data: BanklessDataFor<B>,
    client: &C,
    last_block_in_session: NumberFor<B>,
) -> Option<BanklessDataFor<B>>
where
    B: Block,
    C: HeaderBackend<B>,
{
...
mod tests {
    ...
    use crate::data_io::BanklessData;
        ...
        for i in 0..n {
            for j in i..n {
                let maybe_data = should_finalize(
                    blocks[i],
                    BanklessData::new(blocks[j], j as u64),
                    client.as_ref(),
                    100u64,
                );
                let correct_result = if i == j {
                    ...
                    Some(BanklessData::new(blocks[j], j as u64))
                };
        ...
        for i in 0..=n {
            for j in 0..=n {
                if i != j {
                    let maybe_data = should_finalize(
                        extra_children[i],
                        BanklessData::new(extra_children[j], j as u64),
                        client.as_ref(),
                        100u64,
                    );
                    assert!(maybe_data.is_none());
    ...
    #[test]
    fn should_finalize_for_incorrect_bankless_data() {
        let mut client = Arc::new(TestClientBuilder::new().build());
        ...
        for i in 0..n {
            for j in i..n {
                let maybe_data = should_finalize(
                    blocks[i],
                    BanklessData::new(blocks[j], (j + 1) as u64),
                    client.as_ref(),
                    100u64,
                );
                assert!(maybe_data.is_none());
