...
impl<NI: NetworkIdentity, D: Data> Service<NI, D> {
    ...
    pub fn on_discovery_message(
        ...
        match self.sessions.get_mut(&session_id) {
            ...
            None => {
                debug!(target: "bankless-network", "Received message from unknown session: {:?}", message);
                (None, Vec::new())
            }
        }
    }
            ...
            tokio::select! {
                maybe_command = self.commands_from_user.next() => match maybe_command {
                    Some(command) => match service.on_command(command).await {
                        ...
                        Err(e) => warn!(target: "bankless-network", "Failed to update handler: {:?}", e),
                    },
                ...
                maybe_message = self.messages_from_network.next() => match maybe_message {
                    Some(message) => if let Err(e) = self.on_network_message(&mut service, message) {
                        match e {
                            Error::UserSend => warn!(target: "bankless-network", "Failed to send to user in session."),
                            Error::NoSession => warn!(target: "bankless-network", "Received message for unknown session."),
                            _ => return Err(e),
                        }
                    },
