    ...
    match receiver.lock().await.next().await {
        Some(Split::Left(data)) => {
            if left_sender.unbounded_send(data).is_err() {
                warn!(target: "bankless-network", "Failed send despite controlling receiver, this shouldn't've happened.");
            }
        }
...
///
/// The main example for now is creating an `bankless_bft::Network` and a separate one for accumulating
