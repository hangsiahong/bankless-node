use crate::new_network::{
    ConnectionCommand, Data, DataCommand, Network, PeerId, Protocol, BANKLESS_PROTOCOL_NAME,
    BANKLESS_VALIDATOR_PROTOCOL_NAME,
};
    ...
    fn handle_network_event(&mut self, event: Event) -> Result<(), mpsc::TrySendError<D>> {
        match event {
            Event::SyncConnected { remote } => {
                trace!(target: "bankless-network", "SyncConnected event for peer {:?}", remote);
                let addr = iter::once(multiaddr::Protocol::P2p(remote.into())).collect();
                self.network.add_reserved(
                    iter::once(addr).collect(),
                    Cow::Borrowed(BANKLESS_PROTOCOL_NAME),
                );
            }
            Event::SyncDisconnected { remote } => {
                trace!(target: "bankless-network", "SyncDisconnected event for peer {:?}", remote);
            ...
            Event::NotificationStreamOpened {
                ...
                if protocol == BANKLESS_PROTOCOL_NAME {
            ...
            Event::NotificationsReceived {
                ...
                for (protocol, data) in messages.into_iter() {
                    if protocol == BANKLESS_PROTOCOL_NAME || protocol == BANKLESS_VALIDATOR_PROTOCOL_NAME
                    {
                        match D::decode(&mut &data[..]) {
                            Ok(message) => self.messages_for_user.unbounded_send(message)?,
                            Err(e) => {
                                debug!(target: "bankless-network", "Error decoding message: {}", e)
                            }
                        }
                    }
                }
    ...
    fn on_manager_command(&self, command: ConnectionCommand) {
        ...
        match command {
            AddReserved(addresses) => self
                .network
                .add_reserved(addresses, Cow::Borrowed(BANKLESS_VALIDATOR_PROTOCOL_NAME)),
            DelReserved(peers) => self
                .network
                .remove_reserved(peers, Cow::Borrowed(BANKLESS_VALIDATOR_PROTOCOL_NAME)),
        }
    }
            ...
            tokio::select! {
                maybe_event = events_from_network.next() => match maybe_event {
                    Some(event) => if let Err(e) = self.handle_network_event(event) {
                        error!(target: "bankless-network", "Cannot forward messages to user: {:?}", e);
                        return;
                    },
                    None => {
                        error!(target: "bankless-network", "Network event stream ended.");
                        return;
                    }
                },
                maybe_command = self.commands_from_manager.next() => match maybe_command {
                    ...
                    None => {
                        error!(target: "bankless-network", "Manager command stream ended.");
                        return;
                    }
                },
                maybe_message = self.messages_from_user.next() => match maybe_message {
                    ...
                    None => {
                        error!(target: "bankless-network", "User message stream ended.");
                        return;
                    }
                },
                Some(result) = Self::send(&self.network, &mut self.to_send) => {
                    if let Err(e) = result {
                        debug!(target: "bankless-network", "Failed sending data to peer: {:?}", e);
                    }
                },
            }
        }
...
mod tests {
    ...
    use crate::new_network::{
        ...
        NetworkIdentity, Protocol, BANKLESS_PROTOCOL_NAME, BANKLESS_VALIDATOR_PROTOCOL_NAME,
    };
    use codec::Encode;
    use futures::{
    ...
    async fn test_notification_stream_opened() {
        ...
        identities.iter().for_each(|identity| {
            network.emit_event(Event::NotificationStreamOpened {
                protocol: Cow::Borrowed(BANKLESS_PROTOCOL_NAME),
                remote: identity.1.into(),
        ...
        let expected_messages = HashSet::from_iter(
            identities
                ...
                .map(|identity| {
                    messages
                        .iter()
                        .map(move |m| (m.encode(), identity.1, Cow::Borrowed(BANKLESS_PROTOCOL_NAME)))
                })
                .flatten(),
        );
    ...
    async fn test_command_add_reserved() {
        ...
        let expected = (
            ...
            Cow::Borrowed(BANKLESS_VALIDATOR_PROTOCOL_NAME),
        );
