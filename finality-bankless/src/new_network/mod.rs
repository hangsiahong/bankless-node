...
pub use bankless::NetworkData as BanklessNetworkData;
...
/// Name of the network protocol used by Bankless World validators. Similar to
/// BANKLESS_PROTOCOL_NAME, but only used by validators that authenticated to each other.
const BANKLESS_VALIDATOR_PROTOCOL_NAME: &str = "/cardinals/bankless_validator/1";
...
impl Protocol {
    pub fn name(&self) -> Cow<'static, str> {
        use Protocol::*;
        match self {
            Generic => Cow::Borrowed(BANKLESS_PROTOCOL_NAME),
            Validator => Cow::Borrowed(BANKLESS_VALIDATOR_PROTOCOL_NAME),
        }
    }
}
