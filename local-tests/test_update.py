...
# Path to the pre-update bankless-node binary:
oldbin = os.getenv('OLD_BINARY', join(workdir, 'bankless-node-old'))
# Path to the post-update bankless-node binary:
newbin = os.getenv('NEW_BINARY', join(workdir, 'bankless-node-new'))
# Path to the post-update compiled runtime:
runtime = os.getenv('NEW_RUNTIME', join(workdir, 'bankless_runtime.compact.wasm'))
# Path to the send-runtime binary (which lives in bankless-node/local-tests/send-runtime):
