## Bankless Fork-Off tool

Command-line tool for creating snapshots of a running bankless-bft chains.
...
```bash
bankless-node bootstrap-chain --raw --base-path /data --chain-id a0fnet1 --account-ids <id1,id2,...>  --sudo-account-id <sudo_id> > chainspec.json
```
...
```bash
bankless-node convert-chainspec-to-raw --chain docker/data/chainspec.json
```

Tool will query the target chain for storage pairs (by default "Aura" and "Bankless") and copy them over to the target fork chainspec, which is finally written out to the specified path:
...
* `prefixes`: which storage items to migrate ("Aura" and "Bankless" by default), e.g. , `--prefixes Aura,Bankless,Treasury,Vesting`
