...
//!
//! For full integration with Bankless finality gadget, the `primitives::BanklessSessionApi` should be implemented.
...
pub mod pallet {
    ...
    use primitives::{
        ApiError as BanklessApiError, DEFAULT_MILLISECS_PER_BLOCK, DEFAULT_SESSION_PERIOD,
    };
    ...
    pub struct BanklessSessionManager<T>(sp_std::marker::PhantomData<T>);
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn next_session_authorities() -> Result<Vec<T::AuthorityId>, BanklessApiError> {
            Session::<T>::queued_keys()
                .iter()
                .map(|(_, key)| key.get(T::AuthorityId::ID).ok_or(BanklessApiError::DecodeKey))
                .collect::<Result<Vec<T::AuthorityId>, BanklessApiError>>()
        }
    }

    impl<T: Config> SessionManager<T::AccountId> for BanklessSessionManager<T> {
