...
# bankless_node cli options to env variables
...
if [[ "true" == "$PURGE_BEFORE_START" ]]; then
  ...
  bankless-node purge-chain --base-path "${BASE_PATH}" --chain "${CHAIN}" -y
fi
...
if [[ -n "${FLAG_L_BANKLESS_BFT:-}" ]]; then
  ARGS+=(-lBanklessBFT=debug)
fi
